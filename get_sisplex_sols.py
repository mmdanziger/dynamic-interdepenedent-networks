...
try:
    import sisplex as sis
except ImportError:
    from oscillators import sisplex as sis
...
if mode == "slave":
    ...
    if degree_dist == "sf":
        intfunc = sis.get_sf_integral(k) #actually gamma
    elif degree_dist == "rr":
        intfunc = sis.get_rr_integral(k)
    elif degree_dist == "er":
        if force_fname:
            integral_fname = integral_fname
        elif degree_dist == "er":
            integral_fname = "/home/micha/phd/oscillators/sis_interpolation_values_combined_1d_k%i_HD.json" % k
        ...
        intfunc = sis.get_interpolated_integral(kbar=k, mode="r",fname=integral_fname,gamma=k)
    for lambda1 in sis.np.arange(lambda1_i, lambda1_f, step=lambda_step):
        for lambda2 in sis.np.arange(lambda2_i, lambda2_f, step=lambda_step):
            sols = sis.solve_R1R2_double_solutions(intfunc, [lambda1, lambda2], frac,interaction_type=interaction_type,q=q)
            #print((lambda1,lambda2),end="\t")
            #print(sols,end="\n")
            res["%.8f" % lambda1]["%.8f" % lambda2] = sols
    json.dump(res,
              open("/tmp/sis_two_net_result_%s_k%i_f%.4f_lambdai%.8f_lambdaf%.8f.json" % (interaction_type, k, frac, lambda1_i, lambda1_f), "w"))
else:
    """
    Master
    """
    ...
    """
    d = {}
    for fname in glob("/tmp/sis_two_net_result_%s_k%i_f%.4f_*.json" % (interaction_type,k, frac)):
        subd = json.load(open(fname))
        for key,v in subd.items():
            d[key] = v
        call(["rm" , fname])
    if degree_dist == "er":
        json.dump(d,open("sis_two_net_result_%s_k%i_f%.4f_q%.4f.json"%(interaction_type,k,frac,q),"w"))
    elif degree_dist == "rr":
        json.dump(d, open("sis_two_net_result_rr_%s_k%i_f%.4f_q%.4f.json" % (interaction_type, k, frac, q), "w"))
    else:
        json.dump(d, open("sis_two_net_result_%s_gamma%i_f%.4f_q%.4f.json" % (interaction_type, k, frac,q), "w"))
